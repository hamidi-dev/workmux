...
export const WorkmuxStatusPlugin: Plugin = async ({ $ }) => {
  ...
  // The pane's own session is the first one this client reports. Subagents are
  // created later and must not displace the session used for pane metadata.
  let rootSession: string | undefined;
  let reportedSession: string | undefined;
  let statusQueue = Promise.resolve();

  function writeStatus(status: string, sessionID: string | undefined) {
    const command = sessionID
      ? $`workmux set-window-status ${status} --session-id ${sessionID}`
      : $`workmux set-window-status ${status}`;
    return command.quiet().then(() => {}, () => {});
  }

  function queueStatus(status: string, sessionID: string | undefined) {
    statusQueue = statusQueue.then(
      () => writeStatus(status, sessionID),
      () => writeStatus(status, sessionID),
    );
    return statusQueue;
  }
  ...
  async function reportAggregateStatus() {
    ...
    await queueStatus(status, rootSession);
  }
